...
static int yacd5c1sbdbc_sensor_set_check_zsl(struct msm_sensor_ctrl_t *s_ctrl ,int8_t checkzsl)
{
	int rc = 0;

	SKYCDBG("%s checkzsl=%d start \n",__func__,checkzsl);

	if(checkzsl < 0 || checkzsl > 1){
		SKYCERR("%s error. checkzsl=%d\n", __func__, checkzsl);
		return -EINVAL;
	}
    
    check_zsl_flag = checkzsl;
...
//	.sensor_write_exp_gain = msm_sensor_write_exp_gain_yacd5c1sbdbc, //return 0
    ...
    .sensor_set_check_zsl = yacd5c1sbdbc_sensor_set_check_zsl,    
#endif
};
